...
AlignmentMaps FilesReader::getAlignments(const std::string& fileName, const size_t& from, const size_t& to,
	...
	int refID = reader.GetReferenceID("NC_001416");
	//We are interested in reading only a certain region, not everything at once in order to not have memory exhaustion
	//TODO for some reason mutated_12202_aligned_166_R_24_12985_15 is not retrieved in 24k 32k range even though it is retrieved in the samtools
	if (!reader.SetRegion(refID, from, refID, to)) {
